...
fn signing_message_v2(
	...
	let escrow_id = parse_hex_bytes::<32>(&v2.escrow_id)
		.map_err(|e| format!("escrow_id: {}", e))?;
	let recipient = parse_hex_bytes::<32>(&v2.recipient)
		.map_err(|e| format!("recipient: {}", e))?;
	let nonce = parse_hex_bytes::<16>(&v2.nonce)
